...
function WechatPayment() {
  const payment = useAuth((state) => state.payment)
  const pollPayment = useAuth((state) => state.pollPayment)
  const clearPayment = useAuth((state) => state.clearPayment)
  const [image, setImage] = useState('')

  useEffect(() => {
    if (!payment) return
    let alive = true
    void QRCode.toDataURL(payment.codeUrl, { width: 260, margin: 1, color: { dark: '#10151c', light: '#ffffff' } }).then((value) => alive && setImage(value))
    const timer = window.setInterval(() => { void pollPayment() }, 2500)
    return () => { alive = false; window.clearInterval(timer); setImage('') }
  }, [payment, pollPayment])

  if (!payment) return null
  return <div className="wechat-payment" role="dialog" aria-modal="true" aria-label="微信支付">
    <div className="wechat-payment-card">
      <p className="product-kicker">WECHAT PAY</p>
      <h2>微信扫码支付 ¥3</h2>
      <p>付款成功后将自动解锁编辑器，无需刷新页面。</p>
      {image ? <img src={image} alt="微信支付二维码" /> : <div className="qr-loading">正在生成二维码...</div>}
      <button className="text-action" onClick={clearPayment}>取消本次支付</button>
    </div>
  </div>
}
...
export function ProductPage() {
  return <div className="product-page">
    ...
    <main>
      ...
      <section className="product-proof" id="tools"><p>ONE TEMPLATE, YOUR LANGUAGE</p><div><article><b>01</b><h2>把镜头变成叙事</h2><p>在时间线上安排关键帧，直接控制镜头偏移、焦点、景深与虚化强度。</p></article><article><b>02</b><h2>每一层都能编辑</h2><p>贴纸、个人资料、履历节点与作品内容，都能在同一处可视化调整。</p></article><article><b>03</b><h2>模型不必一样</h2><p>导入自己的 GLB 后，使用模型适配器更新主体、聚焦锚点和展示画面。</p></article></div></section>
