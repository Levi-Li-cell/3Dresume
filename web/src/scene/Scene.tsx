...
import StickerPlanes from '../editor/StickerPlanes'
...
// liwei.rigged.glb：模型 + glb 自带相机动画（滚动分 5 段擦除）+ 自动对焦 + 眼睛跟随
function Man2({
  focusRef,
  frameRef,
  dofBokehRef,
  ...
  // 注册 man 节点（贴纸编辑器用它做坐标对齐 + 脸部拾取）
  const manObj = useMemo(() => model?.getObjectByName('man') ?? null, [model])
  const setMan = useMan((s) => s.setMan)
  useEffect(() => {
    setMan(manObj)
    return () => {
      if (useMan.getState().man === manObj) setMan(null)
    }
  }, [manObj, setMan])

  // 相机动画总帧数：从 CameraAction clip 读（回退到最长 clip / 默认履历+入场+横移），不写死。
  // 作品区帧段 = [RESUME_FRAMES, totalFrames]，长度随 glb 而定（当前 liwei.rigged.glb 为 100 帧）。
  ...
  useFrame((_, dt) => {
    ...
    if (previewFrame !== null) {
      frameTarget = THREE.MathUtils.clamp(previewFrame, 0, totalFrames)
      inWorks = frameTarget > RESUME_FRAMES
    }
    ...
    if (previewFrame !== null) frameSmooth.current = frameTarget
    else frameSmooth.current += (frameTarget - frameSmooth.current) * aEff
    ...
    useDirectorStore.getState().setRuntime({
      frame: Math.round(frame),
      totalFrames,
      bokehScale: dofBokehRef?.current ?? 0,
      focusRange: dofRangeRef?.current ?? 0.15,
    })
...
// 场景根组件：展示 liwei.rigged.glb（相机由 glb 动画 + 滚动驱动）
